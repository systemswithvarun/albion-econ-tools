...
import { Star, Search, Loader2, ArrowLeftRight, Edit2, Check, X, TrendingUp, Info } from 'lucide-react'
...
export function PriceCheckerClient({
  ...
  const handleInlineSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!editingCell || !selectedItem) return

    const { city, side, value } = editingCell
    const priceNum = Number(value)
    if (isNaN(priceNum) || priceNum <= 0) return

    setSubmittingCell({ city, side })
    try {
      const fd = new FormData()
      fd.append('itemId', selectedItem.item_id)
      fd.append('city', city)
  ...
  // Calculate Black Market Profit calculations for selected quality
  const qualityPrices = prices.filter((p) => p.quality === selectedQuality)
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <main className="flex-1 container mx-auto px-6 py-6 grid gap-6 md:grid-cols-[340px_1fr]">
        ...
        {/* Right Side: Price Details Grid */}
        <div className="flex flex-col gap-4">
          {!selectedItem ? (
            <Card className="flex-1 flex flex-col justify-center items-center text-center p-12 min-h-[400px] border-dashed">
              ...
              <h3 className="text-base font-bold mb-1">No Item Selected</h3>
              <p className="text-sm text-muted-foreground max-w-sm">
                Select an item from the catalog on the left to inspect its live prices, daily volume, and check Black Market profit gaps.
              </p>
            </Card>
          ) : loadingPrices ? (
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent">
                      <TableHead className="w-[180px] pl-6">City</TableHead>
                      <TableHead>Buy Price (Sell Order)</TableHead>
                      <TableHead>Sell Price (Buy Order)</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                      ...
                      return (
                        <TableRow key={city} className="hover:bg-muted/30">
                          ...
                          <TableCell className="py-2">
                            {renderCell(city, 'sell_order', buyObs)}
                          </TableCell>

                          {/* Sell Price Cell (Buy Order) */}
                          <TableCell className="py-2">
                            {renderCell(city, 'buy_order', sellObs)}
                          </TableCell>
                        </TableRow>
                      )
                    })}
                    ...
                    {(() => {
                      const bmVolume = volumes.find((v) => v.city === 'BlackMarket')
  ...
  function renderCell(city: string, side: 'buy_order' | 'sell_order', obs: LivePrice | undefined) {
    const isEditing = editingCell?.city === city && editingCell?.side === side
    const isSubmitting = submittingCell?.city === city && submittingCell?.side === side
